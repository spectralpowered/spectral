...
import io.spectralpowered.injector.annotations.injection.CASM;
...
import io.spectralpowered.injector.utils.annotations.ClassDefiner;
...
public class CASMTransformer extends ARemovingTransformer<CASM> {
        ...
        if (annotation.value().length == 0) {
            if (args.length != 1 || !Types.type(ClassNode.class).equals(args[0])) {
                throw new TransformerException(transformerMethod, transformer, "must have one argument (ClassNode)")
                        .help(Codifier.of(transformerMethod).param(null).param(Types.type(ClassNode.class)));
            }
            ...
            if (args.length != 1 || !Types.type(MethodNode.class).equals(args[0])) {
                throw new TransformerException(transformerMethod, transformer, "must have one argument (MethodNode)")
                        .help(Codifier.of(transformerMethod).param(null).param(Types.type(MethodNode.class)));
            }
    ...
    private ClassDefiner<?> isolateMethod(final IClassProvider classProvider, final ClassNode transformer, final MethodNode transformerMethod) {
        try {
            ...
            { //<init>
                MethodVisitor init = classNode.visitMethod(Opcodes.ACC_PUBLIC, Types.MN_Init, Types.MD_Void, null, null);
                init.visitCode();
                init.visitVarInsn(Opcodes.ALOAD, 0);
                init.visitMethodInsn(Opcodes.INVOKESPECIAL, Types.IN_Object, Types.MN_Init, Types.MD_Void, false);
