...
class ScriptInterpreterCleaner : Transformer() {

    private var count = 0

    private val opcodes = ScriptOpcodes::class.java.declaredFields.filter { it.type == Integer.TYPE }.also {
    ...
    override fun transformPool(pool: ClassPool) {
         ...
         */
        val scriptOpcodesCls = ScriptInterpreterCleaner::class.java.getResourceAsStream("/io/spectralpowered/deobfuscator/transformer/ScriptInterpreterCleaner\$ScriptOpcodes.class")!!
            .readAllBytes().let { ClassNode().also { cls -> cls.fromByteArray(it) } }
        scriptOpcodesCls.name = "ScriptOpcodes"
        scriptOpcodesCls.ignored = true
        scriptOpcodesCls.visibleAnnotations.clear()
        scriptOpcodesCls.fields.removeAll { it.name == "INSTANCE" }
        scriptOpcodesCls.methods.removeAll { it.isInitializer() }
        pool.addClass(scriptOpcodesCls)
        scriptOpcodesCls.build()
        ...
        pool.classes.forEach { cls ->
            cls.methods.forEach methodLoop@ { method ->
                ...
                while(insns.hasNext()) {
                    ...
                    val getStatic = FieldInsnNode(GETSTATIC, scriptOpcodesCls.name, opcodeName, "I")
                    insns.set(getStatic)
                    count++
                }
            }
