...
public abstract class ARemovingTargetTransformer<T extends Annotation> extends ARemovingTransformer<T> {
    ...
    public final void transform(T annotation, InjectionManager injectionManager, IClassProvider classProvider, Map<String, IInjectionTarget> injectionTargets, ClassNode transformedClass, ClassNode transformer, MethodNode transformerMethod) {
        for (String targetCombi : this.targetCombis.apply(annotation)) {
            ...
            for (MethodNode target : targets) {
                this.transform(annotation, injectionManager, classProvider, injectionTargets, transformedClass, transformer, ASMUtils.cloneMethod(transformerMethod), target);
            }
        }
    }
     ...
     */
    public abstract void transform(final T annotation, final InjectionManager injectionManager, final IClassProvider classProvider, final Map<String, IInjectionTarget> injectionTargets, final ClassNode transformedClass, final ClassNode transformer, final MethodNode transformerMethod, final MethodNode target);
