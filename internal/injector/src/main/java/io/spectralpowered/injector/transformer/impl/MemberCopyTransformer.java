...
public class MemberCopyTransformer extends ATransformer {
    ...
    private void mergeInitializers(ClassNode transformedClass, ClassNode transformer) {
        ...
        for (MethodNode method : transformedClass.methods) {
            if (!method.name.equals(Types.MN_Init)) continue;
        ...
        if (!unresolvedInitializers.isEmpty()) {
            MethodNode emptyConstructor = ASMUtils.getMethod(transformer, Types.MN_Init, Types.MD_Void);
        ...
        MethodNode staticBlock = ASMUtils.getMethod(transformer, Types.MN_Clinit, Types.MD_Void);
    ...
    private void copyInitializers(final ClassNode fromClass, final MethodNode from, final ClassNode toClass, final MethodNode to) {
...

        {
            ...
            if (from.name.equals(Types.MN_Init)) first = ASMUtils.getFirstConstructorInstruction(fromClass.superName, from);
    ...
    private MethodNode createStaticBlock(final ClassNode transformedClass) {
        ...
        MethodVisitor staticBlock = transformedClass.visitMethod(Opcodes.ACC_STATIC, Types.MN_Clinit, Types.MD_Void, null, null);
