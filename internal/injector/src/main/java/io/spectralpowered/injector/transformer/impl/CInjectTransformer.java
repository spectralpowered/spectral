...
public class CInjectTransformer extends ARemovingTargetTransformer<CInject> {
        ...
        }
        {
                    ...
                    for (int i = annotations.length - 1; i >= 0; i--) {
                        ...
                        if (paramAnnotations != null) {
                            for (AnnotationNode paramAnnotation : paramAnnotations) {
                                if (paramAnnotation.desc.equals(Types.typeDescriptor(CLocalVariable.class))) {
            ...
            if (arguments.length == 0) {
                ...
                throw new TransformerException(transformerMethod, transformer, "must have the same arguments as target method or no arguments with optional InjectionCallback")
                        .help(Codifier.of(target).param(Types.type(InjectionCallback.class)));
            }
        }
    ...
    private void copyBackLocalVars(final MethodNode source, final List<CLocalVariable> localVariables) {
        ...
        Type returnType = Types.returnType(source.desc);
        Type[] parameterTypes = Types.argumentTypes(source.desc);
        ...
        newParameterTypes[newParameterTypes.length - 1] = Types.type(Object[].class);
        source.desc = Types.methodDescriptor(returnType, (Object[]) newParameterTypes);
    ...
    private void createCallback(final InsnList instructions, final boolean cancellable, final boolean hasCallback, final int callbackVar, final Type returnType, final int returnVar) {
        if (hasCallback) { //Create the callback instance
            instructions.add(new TypeInsnNode(Opcodes.NEW, Types.internalName(InjectionCallback.class)));
            ...
            if (!Type.VOID_TYPE.equals(returnType)) {
                ...
                instructions.add(new MethodInsnNode(Opcodes.INVOKESPECIAL, Types.internalName(InjectionCallback.class), Types.MN_Init, Types.methodDescriptor(void.class, boolean.class, Object.class)));
            } else {
                instructions.add(new MethodInsnNode(Opcodes.INVOKESPECIAL, Types.internalName(InjectionCallback.class), Types.MN_Init, Types.methodDescriptor(void.class, boolean.class)));
            }
    ...
    private void callInjectionMethod(final InsnList instructions, final ClassNode classNode, final MethodNode target, final MethodNode source, final List<CLocalVariable> localVariables) {
        ...
        if (!localVariables.isEmpty()) {
            ...
            Type[] parameter = Types.argumentTypes(source.desc);
            for (int i = 0; i < localVariables.size(); i++) {
            ...
            if (modifiableVariableCount > 0) {
                ...
                loadLocals.add(new TypeInsnNode(Opcodes.ANEWARRAY, Types.internalName(Object.class)));
    ...
    private void getCancelInstructions(final InsnList instructions, final boolean cancellable, final boolean hasCallback, final int callbackVar, final Type returnType) {
        if (cancellable && hasCallback) { //If the method is cancellable, check if the callback has been cancelled
            ...
            instructions.add(new VarInsnNode(Opcodes.ALOAD, callbackVar));
            instructions.add(new MethodInsnNode(Opcodes.INVOKEVIRTUAL, Types.internalName(InjectionCallback.class), "isCancelled", Types.methodDescriptor(boolean.class)));
            ...
            if (!Type.VOID_TYPE.equals(returnType)) { //If the method has a return value, take the value from the callback
                instructions.add(new VarInsnNode(Opcodes.ALOAD, callbackVar));
                instructions.add(new MethodInsnNode(Opcodes.INVOKEVIRTUAL, Types.internalName(InjectionCallback.class), "getReturnValue", Types.methodDescriptor(Object.class)));
